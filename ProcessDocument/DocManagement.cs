...
namespace ProcessDocument
{
    ...
    {
        private static int _transportTwrNumer;
        private static decimal progZS;
        private static decimal progZSKemon;
        private static decimal progZSKemonMax;
        private static int progZSMax;
        private static int apiSession;
        private static string token;
        private static int _companyId;

        internal static void PrzetworzZam(Model.Zamowienie zam, List<RejestrPH> rejestryPH, out bool czekaNaWspolne)
        {
            ...
            if (doc.Head.GIDType == 1603)
                tmpMMW = null;// API.PobierzDokument(doc.Head.GIDType, doc.Head.GIDNumber);
        ...
        private static void WystawGratis(Model.Zamowienie zam, Document doc, int zamId, DocumentPos p)
        {
            var wystarczajacyStan = SQL.SprawdzStan(p.TwrNumber, "001", p.Quantity);
            if (!wystarczajacyStan)
            {
                SetLogText($"Brakuje stanu dla pozycji gratisowej {p.TwrCode} w ilości {p.Quantity} - pomijanie");
            }
            else
            {
                SetLogText($"Dodawanie pozycji gratisowej {p.TwrCode} w ilości {p.Quantity} w cenie {p.Price}");
                ZamPoz gratisPos = NewGratis(zam, p);
                var lp = 0;// API.DodajPozycjeZam(zamId, ref gratisPos);
                DocumentPos wzPos = NewZamPos(zam, doc, p, lp);
                doc.Positions.Add(wzPos);
            }
        }

        private static void SetLogText(string v)
        {
            throw new NotImplementedException();
        }

        private static DocumentPos NewZamPos(Model.Zamowienie zam, Document doc, DocumentPos p, int lp)
        {
            return new DocumentPos
            {
                GIDType = doc.Head.GIDType,
                GIDFirm = zam.GIDFirm,
                TwrNumber = p.TwrNumber,
                TwrCode = p.TwrCode,
                Quantity = p.Quantity,
                Price = p.Price,
                ZlcType = zam.GIDType,
        ...
        private static ZamPoz NewGratis(Model.Zamowienie zam, DocumentPos p)
        {
            return new XLAPI_Wrapper.ZamPoz
            {
                GIDTyp = zam.GIDType,
                GIDFirma = zam.GIDFirm,
                GIDNumer = zam.GIDNumer,
                TwrTyp = 16,
                TwrFirma = zam.GIDFirm,
                TwrNumer = p.TwrNumber,
                Ilosc = p.Quantity,
                Cena = p.Price,
                CenaPoczatkowa = p.StartPrice
            };
        }

        public static void SendMail(string To, string Subject, string Body)
        {
            using (var mail = new MailMessage(Properties.Settings.Default.MailFrom, To.Replace(";", ","), Subject, Body))
            {
                mail.Bcc.Add(Properties.Settings.Default.MailFrom);
                ...
                using (var smtp = new SmtpClient(Properties.Settings.Default.SMTPHost, Properties.Settings.Default.SMTPPort))
                {
                    ...
                    smtp.Credentials = new System.Net.NetworkCredential(Properties.Settings.Default.SMTPLogin, Properties.Settings.Default.SMTPPassword);
                    smtp.Send(mail);
                }
            }
        }

        internal static void TryMultipleTimes(Action action, int maxCount)
        {
            var count = 0;
